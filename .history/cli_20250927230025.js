...
async function preprocessCode(filePath, processedFiles = new Set()) {
    ...
    if (!fs.existsSync(absolutePath)) {
        // --- NEW: Throw a clear error if the entry file doesn't exist ---
        throw new Error(`\n💥 FATAL WORD CRIME: ImportError 💥\n\nAl says: "I couldn't find the file to run!"\n\n   at path: ${absolutePath}\n`);
    }

    let code = fs.readFileSync(absolutePath, 'utf-8');
    
    // Use a regular expression that finds all forms of #eat directives for user files
    const eatRegex = /^#eat\s*(?:"([^"]+)"|([a-zA-Z0-9_.\-/]+\.(?:hat|yc)))/gm;
    let match;
    const imports = [];
    while ((match = eatRegex.exec(code)) !== null) {
        // match[1] is for quoted, match[2] is for bare
    ...
    for (const imp of imports) {
        ...
        // If a user-defined import is not found, throw a fatal error.
        if (!fs.existsSync(importPath)) {
            throw new Error(`\n💥 FATAL WORD CRIME: ImportError 💥\n\nAl says: "I went to the hardware store, but couldn't find this file!"\n\n   Required by: ${absolutePath}\n   Missing file: ${importPath}\n`);
        }
        
        const importedCode = await preprocessCode(importPath, processedFiles);
        // Replace the import statement with the file's content
