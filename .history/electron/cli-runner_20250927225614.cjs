...
`);
    ...
    // Close the window and quit the app with an error code
...
ipcMain.on('program-finished', () => {
    console.log('[CLI Runner] Program finished. Closing window.');
    // Get the window that sent the message and close it.
    const window = BrowserWindow.getFocusedWindow();
    if (window) {
        window.close();
    }
    app.quit(0); // Clean exit
});
