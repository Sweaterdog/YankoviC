...
const initializeCliMain = async () => {
    ...
    const interpreter = new YankoviCInterpreter({
        ...
        printFunction: (text) => console.log(text)
    });
        ...
        try {
            ...
            if (interpreter.polkaLoop) {
                ...
                const runAnimationLoop = async () => {
                    // Also wrap the frame execution
                    try {
                        if (!interpreter.polkaLoop) return;
                        await interpreter.runFrame();
                        requestAnimationFrame(runAnimationLoop);
                    } catch (error) {
                         // Report frame-level errors
                        console.error('[CLI-MAIN] Fatal error during animation frame:', error);
                        window.cliAPI.reportError({ message: error.message, stack: error.stack });
                    }
                };
                requestAnimationFrame(runAnimationLoop);
            } else {
                console.log('[CLI-MAIN] No polka loop found, program finished cleanly.');
            }
        } catch (error) {
            console.error('[CLI-MAIN] Fatal error running code:', error);
            // Use the API to send the error back to the main process
            window.cliAPI.reportError({ 
                message: error.message.includes('FATAL WORD CRIME') ? error.message : `FATAL WORD CRIME: ${error.name} - ${error.message}`,
                stack: error.stack 
            });
        }
    } else {
