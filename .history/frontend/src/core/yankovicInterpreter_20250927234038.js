...
export class YankoviCInterpreter {
    ...
    createYankovicError(name, message, context = {}) {
        const { token, line: lineNum, column: colNum, code: fullCode } = context;
        const line = token ? token.line : lineNum;
        const column = token ? token.column : colNum;
        const code = this.code || fullCode;

        let errorMessage = `\n💥 FATAL WORD CRIME: ${name} 💥\n\n`;
        errorMessage += `Al says: "${message}"\n`;

        if (line && column && code) {
            const codeLines = code.split('\n');
            const offendingLine = codeLines[line - 1];

            errorMessage += `\n   at line ${line}, column ${column}:\n`;
            errorMessage += `   > ${offendingLine}\n`;
            errorMessage += `     ${' '.repeat(column)}^-- Right here, you weasel...\n`;
        }

        // --- NEW: Thematic Hints ---
        let hint = "";
        if (message.includes("missing a semicolon")) {
            hint = "Every good verse needs a proper end. Try adding a ';' to the line above.";
        } else if (message.includes("Unexpected character")) {
            hint = "That character looks about as welcome as a polka band at a funeral. Are you sure it belongs?";
        } else if (message.includes("Expected")) {
            hint = "It's like you're trying to rhyme 'orange' with 'door hinge'. The syntax here feels a bit off.";
        } else if (name === "ReferenceError") {
            hint = "Did you declare that variable? It's about as easy to find as a sane person in Albuquerque.";
        } else if (message.includes("is not a function")) {
            hint = "You're calling that like it's a function, but it's more of a twinkie-wiener sandwich. Make sure it's defined correctly.";
        } else if (name === 'ParseError' && message.includes("Expected TYPE_KEYWORD")) {
            hint = "Are you trying to use a lunchbox type here? Make sure it was defined before this point in the code. You wouldn't put the milk in cereal first!";
        }
        ...
        error.name = name; // Keep original name for logic
        return error;
    }
    ...
    initializeStandardLibrary() {
        this.globalScope.set('perform_a_parody', {
            ...
            call: (args) => {
                ...
                let argIndex = 1;
                let result = formatString.replace(/%(\w+)/g, (match, type) => {
    ...
    lexer(code) {
        ...
        while (position < code.length) {
            ...
            for (const [regex, type] of tokenRegexes) {
                ...
                if (result) {
                    match = { value: result[0], type, raw: result[1] };
                    break;
                }
            }

            if (!match) {
                // --- NEW: Throw with more context ---
                const column = position - lineStart + 1;
                throw this.createYankovicError('SyntaxError', `Unexpected character: '${code[position]}'`, { line, column, code });
            }

            // --- NEW: Add position info to tokens ---
            if (match.type) {
                const column = position - lineStart + 1;
                const tokenValue = match.type === 'STRING' ? match.raw || match.value.slice(1, -1) : match.value;
                const token = { type: match.type, value: tokenValue, line, column }; // Add position
                tokens.push(token);
            }

            // --- NEW: Update line and column for the next token ---
            const newlines = (match.value.match(/\n/g) || []).length;
            if (newlines > 0) {
                line += newlines;
                lineStart = position + match.value.lastIndexOf('\n') + 1;
            }
            position += match.value.length;
        }
        tokens.push({ type: "EOF", value: "EOF", line, column: 1 });
        return tokens;
    }
    ...
    consume(type, value = null) {
        ...
        const message = `Expected ${type} ('${value || 'any'}') but got ${token.type} ('${token.value}')`;
        throw this.createYankovicError('ParseError', message, { token });
    }
    ...
    parseTopLevelDeclaration() {
        ...
        // --- THIS IS THE CORRECTED LOOKAHEAD LOGIC ---
        const peek1 = this.peekToken(1);
        const peek2 = this.peekToken(2);
        
        // A function can start with a primitive type OR a user-defined type (IDENTIFIER),
        // followed by an identifier and an open parenthesis.
        const isFunction = 
            ((token.type === 'TYPE_KEYWORD' || token.type === 'IDENTIFIER') && peek1?.type === 'IDENTIFIER' && peek2?.value === '(') ||
            (token.type === 'VISIBILITY_KEYWORD' && (this.peekToken(1)?.type === 'TYPE_KEYWORD' || this.peekToken(1)?.type === 'IDENTIFIER') && this.peekToken(2)?.type === 'IDENTIFIER' && this.peekToken(3)?.value === '(');

        if (isFunction) {
            return this.parseFunctionDeclaration();
        }
        
        // If it's not a directive, lunchbox, or function, it must be a top-level statement (which might be an error).
        return this.parseStatement();
    }

    instantiateLunchbox(typeName, scope) {
        const definition = scope.get(typeName);
        if (!definition || definition.type !== 'LunchboxDefinition') {
            throw this.createYankovicError('TypeError', `Unknown lunchbox type '${typeName}'.`, {});
        }

        const instance = {};
        for (const field of definition.fields) {
            // Check if the field's type is another lunchbox
            if (this.declaredLunchboxTypes.has(field.type)) {
                // It is! Recursively instantiate it.
                instance[field.name] = this.instantiateLunchbox(field.type, scope);
            } else {
                // It's a primitive, initialize to a default value.
                switch (field.type) {
                    case 'spatula': instance[field.name] = 0; break;
                    case 'lasagna': instance[field.name] = 0.0; break;
                    case 'verse': instance[field.name] = ""; break;
                    case 'horoscope': instance[field.name] = false; break;
                    default: instance[field.name] = null;
                }
            }
        }
        return instance;
    ...
    parseLunchboxDeclaration() {
        ...
        this.declaredLunchboxTypes.add(name); // Keep track of defined types
        this.consume('PUNCTUATION', '{');
        const fields = [];
        while(this.currentToken().value !== '}') {
            const fieldTypeToken = this.consume(this.currentToken().type === 'TYPE_KEYWORD' ? 'TYPE_KEYWORD' : 'IDENTIFIER');
            const fieldName = this.consume('IDENTIFIER').value;
            fields.push({ name: fieldName, type: fieldTypeToken.value });
            this.consume('PUNCTUATION', ';');
        }
        this.consume('PUNCTUATION', '}');
    ...
    parseFunctionDeclaration() {
        ...
        // --- THIS IS THE FIX ---
        // Allow the return type to be a primitive OR a user-defined lunchbox identifier
        const returnTypeToken = this.consume(this.currentToken().type === 'TYPE_KEYWORD' ? 'TYPE_KEYWORD' : 'IDENTIFIER');
        ...
        if (this.currentToken().value !== ')') {
            do {
                if(this.currentToken().value === ',') this.consume('PUNCTUATION', ',');
                // Also fix parameter types here for consistency!
    ...
    parseStatement() {
        ...
        if (token.type === 'CONST_KEYWORD' || token.type === 'TYPE_KEYWORD' || 
            (token.type === 'IDENTIFIER' && this.declaredLunchboxTypes.has(token.value))) {
        ...
        const message = `Unexpected token at start of statement: '${token.value}'`;
        throw this.createYankovicError('ParseError', message, { token });
    }
    ...
    parseHardwareStoreStatement() {
        ...
        let init = null;
        if (this.currentToken().value !== ';') {
            // Check if it's a full declaration or just an expression
            if (this.currentToken().type === 'CONST_KEYWORD' || this.currentToken().type === 'TYPE_KEYWORD' ||
                (this.currentToken().type === 'IDENTIFIER' && this.declaredLunchboxTypes.has(this.currentToken().value))) {
                init = this.parseVariableDeclaration(true); // isForLoopInit = true
            } else {
                init = this.parseExpression();
            }
        }
        this.consume('PUNCTUATION', ';'); // Consume the semicolon AFTER the initializer
        ...
        this.consume('PUNCTUATION', ';'); // Consume the semicolon AFTER the test condition
        ...
        this.consume('PUNCTUATION', ')'); // Consume the closing parenthesis
    ...
    async interpret(node, scope) {
         ...
         switch (node.type) {
            ...
            case 'VariableDeclaration':
                let value;
                // If an initializer is provided, evaluate it.
                if (node.init) {
                    value = await this.interpret(node.init, scope);
                } 
                // If no initializer AND it's a lunchbox type, create a default instance.
                else if (this.declaredLunchboxTypes.has(node.varType)) {
                    value = this.instantiateLunchbox(node.varType, scope);
                } 
                // Otherwise, it's a primitive with no initializer (value is undefined).
                else {
                    value = undefined;
                }
    ...
    async runFrame() {
        ...
        this.frameCount++;
        this.isFrameRunning = true; // <--- CORRECTED
        ...
        try {
            ...
            if (testResult) {
                ...
                this.isFrameRunning = false; // <--- CORRECTED
    ...
    stopLoop() {
        if (!this.polkaLoop) return;
        // In the IDE context, this talks to main.cjs
        if (typeof window !== 'undefined' && window.uhfAPI) {
            window.uhfAPI.cancelTheShow();
        }
        // --- NEW: Add a signal for the CLI runner ---
        if (typeof window !== 'undefined' && window.cliAPI) {
            window.cliAPI.programFinished();
        }
        this.polkaLoop = null;
        this.showIsOver = true;
    }

    async run(code, entryFilePath = null, projectName = 'default-project') {
        this.outputBuffer = [];
        this.pos = 0;
        this.polkaLoop = null;
        ...
        try {
            ...
            exitCode = 1; // Set the exit code
            // --- THIS IS THE KEY CHANGE ---
            if (!e.message.startsWith('\n💥')) {
                // If it's not already one of our formatted errors, format it.
                const formattedError = this.createYankovicError(e.name, e.message);
                this.log(formattedError.message); // Log the formatted error for DevTools
                throw formattedError; // **RE-THROW THE ERROR**
            } else {
                // It's already a formatted error, just log and re-throw.
                this.log(e.message);
    ...
    async processImport(directive, scope) {
        const match = directive.value.match(/#eat\s*(?:<(.+?)>|"([^"]*)"|([a-zA-Z0-9_.\-/]+\.(?:hat|yc)))/);
        if (!match) { return; }

        let importIdentifier = match[1] || match[2] || match[3];
        const normalized = importIdentifier.toLowerCase().replace(/\.(hat|yc)(\.js)?$/, '');

        if (normalized === 'uhf' && Object.keys(this.libraryOverrides).length > 0) {
            return;
        }

        if (match[2] && !importIdentifier.endsWith('.hat')) { 
            throw this.createYankovicError('ImportError', 'Quoted imports only work for .hat files.', { token: directive });
        }
        ...
        let importPath = importIdentifier;
        // The 'path' module won't exist in the browser, so we check for its absence.
        // If we are in the browser (IDE), we don't resolve paths, we just use them as is.
        // The fileApiService will treat them as relative to the project root.
        if (typeof path !== 'undefined' && this.currentFilePath && !path.isAbsolute(importPath)) {
             importPath = path.resolve(path.dirname(this.currentFilePath), importPath);
        }
        ...
        try {
            ...
            // In the IDE, this.projectName is set. In CLI, it's not.
            // The fileApiService needs the project name.
            const content = await getFileContent(this.projectName || 'default-project', importPath);
            const oldState = { pos: this.pos, tokens: this.tokens, currentFilePath: this.currentFilePath, projectName: this.projectName };
            
            // Set context for the imported file
            this.tokens = this.lexer(content);
            this.pos = 0;
            this.currentFilePath = importPath;
            this.projectName = oldState.projectName; // Keep the same project context

            // Interpret the imported code
            const ast = this.parseProgram();
            await this.interpret(ast, scope);

            parentDeclaredLunchboxTypes.forEach(type => this.declaredLunchboxTypes.add(type));
            
            // Restore state
            this.tokens = oldState.tokens;
            this.pos = oldState.pos;
            this.currentFilePath = oldState.currentFilePath;
            this.projectName = oldState.projectName;

        } catch (error) {
            console.error(`[YankoviC Import] Error importing file: ${importPath}`, error);
            throw this.createYankovicError('ImportError', `Failed to import user file '${importIdentifier}'.`, { token: directive });
        }
    }
