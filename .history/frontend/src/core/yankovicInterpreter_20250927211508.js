    ...
    async interpret(node, scope) {
         ...
         switch (node.type) {
            ...
            case 'VariableDeclaration':
                let value;
                if (this.declaredLunchboxTypes.has(node.varType)) {
                    // Instantiate it, creating a nested object structure.
                    value = this.instantiateLunchbox(node.varType, scope);
                } else {
                    // Otherwise, handle it as a primitive.
                    value = node.init ? await this.interpret(node.init, scope) : undefined;
                }
