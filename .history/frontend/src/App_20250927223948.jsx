...
function App() {
  ...
  const runCode = useCallback(async (codeToRun, filePath, project) => {
    ...
    s(prev => prev + `> Running ${filePath || 'code'}...\n`);
    ...
    try {
        const result = await interpreter.run(codeToRun, filePath, project);
        ...
        s(prev => prev + `> Error running code: ${error.message}\n`);
        setDebuggerState('disappointed');
    }
  }, [isLoopRunning, projectName]); // Keep projectName, runCode itself doesn't need activeFile

  // --- THIS IS THE CRITICAL CHANGE ---
  // The function that *calls* runCode needs to be a callback that depends on activeFile
  const handleRunFromConsole = useCallback(() => {
      runCode(activeFile.content, activeFile.path, projectName);
  }, [runCode, activeFile, projectName]);
