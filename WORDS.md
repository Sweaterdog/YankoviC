...
|---|---|
| `#eat <library.hat>` | Imports a standard library (angle brackets) |
| `#eat "filename.yc"` | Imports a user file (quotes) |
| `#eat filename.yc` | Imports a user file (bare name) |

## Comments

| Syntax | Description |
|---|---|
| `//` | Single-line comment. The only valid comment syntax in YankoviC |
...
| `draw_a_button(id,x,y,w,h,text)` | `CreateWindow("BUTTON", ...)` | Draws a clickable button UI element. |
| `button_was_clicked(id)` | `(msg.message == WM_LBUTTONUP)` | Checks if a button with a specific ID has been clicked in the last frame. |
| `draw_a_checkbox(id,x,y,text)` | `CreateWindow("CHECKBOX", ...)` | Draws a checkbox UI element. |
| `get_checkbox_value(id)` | `IsDlgButtonChecked(...)` | Gets the checked state of a checkbox. |
| `draw_a_slider(id,x,y,min,max,value)` | `CreateWindow("SLIDER", ...)` | Draws a slider UI element. |
| `get_slider_value(id)` | `SendMessage(hwnd, TBM_GETPOS, ...)` | Gets the current value of a slider. |
...
| `yoda(a, b)` | `a % b` | Built-in modulus function. Use this instead of the % operator. |
